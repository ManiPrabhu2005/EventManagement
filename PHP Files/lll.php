...
$venueId = $venueName = $venuePrice = $venueCapacity = $venueFacilities = $venueDescription = $imagePath = "";
$venueIdErr = $venueNameErr = $venuePriceErr = $venueCapacityErr = $venueFacilitiesErr = $venueDescriptionErr = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    var_dump($_POST);  // Check if venueFacilities is being passed correctly

    // Validate form fields
    if (empty($venueId)) { $venueIdErr = "Venue ID is required"; }
    ...
    if (empty($venueIdErr) && empty($venueNameErr) && empty($venuePriceErr) && empty($venueCapacityErr) && empty($venueFacilitiesErr) && empty($venueDescriptionErr)) {
        // SQL query to insert the data into the database
        $sql = "INSERT INTO venues (venue_id, venue_name, venue_price, venue_capacity, venue_facilities, venue_description, venue_image)
                VALUES (?, ?, ?, ?, ?, ?, ?)";

        // Prepare and bind the statement
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("ssiiiss", $venueId, $venueName, $venuePrice, $venueCapacity, $venueFacilities, $venueDescription, $imagePath);

            // Execute the query
            if ($stmt->execute()) {
                echo "<h2 class='success-message'>Venue Details Added Successfully!</h2>";
            } else {
...
<div class="container">
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
        <div class="form-section">
            ...
            <input type="text" id="venueId" name="venueId" value="<?php echo htmlspecialchars($venueId); ?>" required>
            <span class="error"><?php echo $venueIdErr; ?></span>
